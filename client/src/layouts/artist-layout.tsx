...
export default function ArtistLayout({ children }: ArtistLayoutProps) {
  ...
  useEffect(() => {
    ...
    } else if (location.includes("/artist/agenda")) {
      setActiveItem("agenda");
    } else if (location.includes("/artist/find-clubs")) {
      setActiveItem("recherche");
    } else if (location.includes("/artist/invitations")) {
  ...
  const checkAuth = () => {
    ...
    try {
      ...
      if (userData.role !== 'artist') {
        console.error('Accès non autorisé: rôle artiste requis');
        window.location.href = "/auth";
        return null;
      }
      return userData;
