...
export interface Product {
  ...
  isAvailable: boolean;
  imageUrl: string;
  destination?: string; // Nouveau champ pour la destination
}
...
const ProductModal = ({
  ...
  const [product, setProduct] = useState<Omit<Product, 'id'> & { id?: number }>({
    ...
    isAvailable: true,
    imageUrl: '',
    destination: 'Cuisine' // Valeur par défaut pour le nouveau champ
  });
  ...
  useEffect(() => {
    if (editingProduct) {
      ...
      setProduct({
        ...
        isAvailable: true,
        imageUrl: '',
        destination: 'Bar' // Valeur par défaut pour les nouveaux produits
      });
    }
    setError(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mapper le champ 'destination' vers 'destinations' pour le backend
      if (product.destination) {
        productToSave.destinations = product.destination;
      }
