...
import {
  ...
  CheckCircle,
  XCircle,
  Loader2 // Ajout pour indicateurs de chargement
} from 'lucide-react';
import {
  Table,
  TableBody,
...
import { Skeleton } from "../../components/ui/skeleton"; // Ajout pour skeletons de chargement (améliore la présentation)
import { motion, AnimatePresence } from "framer-motion"; // Ajout pour animations fluides (rendre plus présentable)
...
// Données fictives (légèrement étendues pour plus de variété)
const events: Event[] = [
  {
    id: 1,
  ...
  // Ajout d'un événement pour plus de démo
  {
    id: 6,
...
const artists: Artist[] = [
  ...
  // Ajout d'un artiste pour plus de démo
  {
    id: 6,
    name: "Tropical Beats",
...
const initialInvitations: Invitation[] = [
  ...
  // Ajout d'une invitation pour plus de démo
  {
    id: 6,
    artistId: 6,
    artist: artists[5],
    ...
    endTime: "03:00",
    fee: 90000,
    createdAt: new Date(2025, 10, 6), // Très récent
    readByArtist: true,
    contractSigned: false
  }
];

// Config stats (inchangée)
...
const ClubInvitationsPage = () => {
  ...
  const [loading, setLoading] = useState(true); // Ajout pour simulation de chargement
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // Pour désactiver boutons pendant "soumission"

  // Simulation de chargement au montage (pour présentation fluide)
  useEffect(() => {
  ...
  // Filtrage (inchangé, mais avec loading)
  ...
  // Handler d'envoi amélioré avec loading
      ...
      <div className="p-8">
        ...
        >
          ...
          <Button
            ...
            className="mt-4 lg:mt-0 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700" // Amélioration design
          >
        ...
        {/* Liste des invitations avec animations */}
        <AnimatePresence>
          <motion.div 
            initial={{ opacity: 0 }} 
            animate={{ opacity: 1 }} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredInvitations.length > 0 ? (
                filteredInvitations.map((invitation, index) => (
                  ...
                  >
                    <Card className="overflow-hidden hover:shadow-lg transition-shadow duration-300"> {/* Ajout hover pour interactivité */}
                      <CardHeader className="pb-3">
                        <div className="flex justify-between items-start">
                          <div>
        ...
        {/* Modal tableau (améliorée avec animations) */}
        ...
        {/* Dialog nouvelle invitation (avec loading) */}
        ...
        {/* Dialog détails artiste (améliorée) */}
