...
import { api, UserRole, InsertUser, InsertArtist, InsertClub } from "@shared/schema";
...
export default function AuthPage() {
  ...
  // Fonction utilitaire pour créer un utilisateur fictif s'il n'existe pas (pour démo)
  const createIfNotExists = async (
    ...
    try {
      ...
      // Utilisateur existe déjà, rien à faire
    } catch (e: any) {
      if (e.message.includes('404') || e.message.includes('not found')) {
        const userData: InsertUser = {
          username,
          password, // Mot de passe en clair ; le backend doit le hasher
          email,
          firstName,
          lastName,
          role,
          // Autres champs par défaut (city, country, etc.)
        };
        ...
        if (role === 'artist') {
          const artistData: InsertArtist = {
            ...
            genre: 'Electronic',
            bio: 'Artiste démo pour la plateforme Bebit.',
            rate: 350,
            tags: ['House', 'Techno'],
            popularity: 50,
          };
          ...
          const clubData: InsertClub = {
            userId: user.id,
            name: `${firstName} ${lastName || ''}`.trim() || username,
            city: 'Paris',
            country: 'France',
            address: 'Adresse démo pour le club.',
            capacity: 300,
            description: 'Club démo pour la plateforme Bebit.',
            latitude: 48.8566, // Coordonnées démo Paris
            longitude: 2.3522,
            // Autres champs par défaut
          };
          await api.createClub(clubData);
        }
        // Pour admin, rien de plus
      } else {
  ...
  const onLoginSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      // Décommentez ce bloc pour activer la création automatique des utilisateurs fictifs
      // (utile pour les premières exécutions de démo ; commentez pour éviter les créations futures)
      await createIfNotExists('user1', 'password123', 'user1@example.com', 'John', 'Doe', 'user');
      await createIfNotExists('dj_elektra', 'password123', 'dj_elektra@example.com', 'Elektra', '', 'artist');
      await createIfNotExists('club_oxygen', 'password123', 'club_oxygen@example.com', 'Oxygen', '', 'club');
      await createIfNotExists('admin', 'adminpass123', 'admin@example.com', 'Admin', '', 'admin');
      

      // Utilisation de l'API pour récupérer l'utilisateur (assume que le backend gère l'authentification réelle via un endpoint dédié)
      // Pour démo : on utilise getUserByUsername ; en prod, remplacez par un appel POST /api/auth/login avec credentials pour token
      const user = await api.getUserByUsername(data.username);

      // Stockage en localStorage pour simulation d'auth (en prod : stockez le token JWT)
      ...
      // Simuler un délai pour l'animation de chargement, puis rediriger
      setTimeout(() => {
        setRedirectTo(path);
      }, 1500);
    } catch (err: any) {
      // Note : Vérification mot de passe devrait être backend-side ; ici, on assume échec si utilisateur non trouvé
      // Pour démo avec mots de passe fictifs identiques, on ignore la vérif client-side
  ...
  const onRegisterSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      const userData: InsertUser = {
        ...
        email: data.email,
        password: data.password, // En clair ; backend hash
      ...
      // Créer profil spécifique si artist ou club
      let path = '/';
      if (data.role === 'artist') {
        const artistData: InsertArtist = {
          userId: user.id,
          ...
          genre: 'Electronic',
          bio: `Bio pour ${data.username}. Complétez votre profil !`,
          rate: 0,
          tags: [],
          popularity: 0,
        };
        ...
        const clubData: InsertClub = {
          ...
          city: 'Paris', // Démo ; en prod, ajoutez champ formulaire
          country: 'France',
          capacity: 200, // Démo
          description: `Description pour ${data.username}. Complétez votre profil !`,
          // Autres par défaut
        };
        await api.createClub(clubData);
        path = '/club';
      ...
      // Simuler délai puis rediriger
      setTimeout(() => {
        setRedirectTo(path);
      }, 1500);
    } catch (err: any) {
      alert(err.message || "Erreur lors de l'inscription. Nom d'utilisateur ou email peut-être déjà pris.");
    } finally {
      setIsLoading(false);
    }
  };
