...
// ======================
// Events table (ADAPTATION: +status et +ticketTypesConfig jsonb pour UI sans nouvelle table)
// ======================
export const events = pgTable("events", {
  ...
  // ADAPTATION: status pour UI (upcoming/planning/past/cancelled)
  status: text("status", { enum: ["upcoming", "planning", "past", "cancelled"] }).notNull().default("planning"),
  // ADAPTATION: ticketTypesConfig jsonb pour config types (name, price, available) ; sold computé via tickets
...
// ======================
// Tickets table (ADAPTATION: +ticketType text pour lier à config jsonb dans events)
// ======================
export const tickets = pgTable("tickets", {
  id: serial("id").primaryKey(),
  eventId: integer("event_id").references(() => events.id).notNull(),
  userId: integer("user_id").references(() => users.id).notNull(),
  // ADAPTATION: ticketType pour catégoriser (match name dans ticketTypesConfig de l'event)
...
// ======================
// Relations (pas de nouvelles ; computations pour ticketTypes via jsonb + groupBy en query)
// ======================
// Users relations
...
// ======================
// Zod schemas for validation (ADAPTATION: validations pour nouveaux champs)
// ======================
...
export const insertEventSchema = createInsertSchema(events, {
  // ADAPTATION: valider status enum
  status: (schema) => schema.refine((val) => ["upcoming", "planning", "past", "cancelled"].includes(val), "Statut invalide"),
  // ADAPTATION: valider ticketTypesConfig comme array d'objets valides
  ticketTypesConfig: (schema) => schema.refine((val) => Array.isArray(val) && val.every(t => typeof t.name === 'string' && typeof t.price === 'number' && typeof t.available === 'number'), "Config types invalide"),
});
export const insertEventArtistSchema = createInsertSchema(eventArtists);
export const insertInvitationSchema = createInsertSchema(invitations);
export const insertTicketSchema = createInsertSchema(tickets, {
  // ADAPTATION: valider ticketType (exemples ; étendre si besoin)
  ticketType: (schema) => schema.refine((val) => ["standard", "vip", "premium"].includes(val), "Type de ticket invalide"),
});
...
// ======================
// Export types (ADAPTATION: types mis à jour pour nouveaux champs)
// ======================
