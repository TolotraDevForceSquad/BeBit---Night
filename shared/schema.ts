...
export const artists = pgTable("artists", {
  ...
  unavailableDates: jsonb("unavailable_dates").default("[]")
});
...
export const transactions = pgTable("transactions", {
  ...
  // NOUVEAUX CHAMPS pour identifier la source
  sourceType: text("source_type", {
    enum: ["ticket_purchase", "table_reservation", "artist_payment", "club_payment", "refund", "withdrawal", "deposit", "promotion", "fee", "event_creation", "invitation_fee"]
  }),
  sourceId: integer("source_id"), // ID de l'entité source (event_id, ticket_id, etc.)
  sourceReference: text("source_reference"), // Référence lisible
...
export const posTables = pgTable("pos_tables", {
  ...
  clubId: integer("club_id").references(() => clubs.id).notNull(),
  clubTableId: integer("club_table_id").references(() => clubTables.id),
});
...
export const orders = pgTable("orders", {
  ...
  clubId: integer("club_id").references(() => clubs.id).notNull(),
  paymentMethodId: integer("payment_method_id").references(() => posPaymentMethods.id),
});
...
export const insertTransactionSchema = createInsertSchema(transactions, {
  ...
  sourceType: (schema) => schema.refine((val) =>
    ["ticket_purchase", "table_reservation", "artist_payment", "club_payment", "refund", "withdrawal", "deposit", "promotion", "fee", "event_creation", "invitation_fee"].includes(val),
    "Type de source invalide"
  ).optional(),
  sourceId: (schema) => schema.optional(),
  sourceReference: (schema) => schema.optional(),
});
...
// POS schemas → avec les nouvelles contraintes
export const insertEmployeeSchema = createInsertSchema(employees, {
  clubId: (schema) => schema.int().min(1, "Club ID requis"),
});
...
export const insertPosTableSchema = createInsertSchema(posTables, {
  clubId: (schema) => schema.int().min(1, "Club ID requis"),
  clubTableId: (schema) => schema.optional(),
});

export const insertOrderSchema = createInsertSchema(orders, {
  clubId: (schema) => schema.int().min(1, "Club ID requis"),
  paymentMethodId: (schema) => schema.optional(),
});
...
// Types pour les sources de transaction
