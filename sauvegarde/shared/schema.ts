...
// POS History - CORRIGÉ: userId en integer au lieu de text
...
// POS relations
export const employeesRelations = relations(employees, ({ many }) => ({ posHistory: many(posHistory) }));
export const posDevicesRelations = relations(posDevices, () => ({}));
export const productCategoriesRelations = relations(productCategories, ({ many }) => ({ products: many(products) }));
export const productsRelations = relations(products, ({ one, many }) => ({
  category: one(productCategories, { fields: [products.categoryId], references: [productCategories.id] }),
  orderItems: many(orderItems),
}));
export const posTablesRelations = relations(posTables, ({ many }) => ({ orders: many(orders), posHistory: many(posHistory) }));
